...
while True:
    ...
    # Wait for 1 ms and check if 's' key is pressed to save the image
    key = cv2.waitKey(1)
    if key == ord('s'):
        print("Save key pressed.")
        if imgWhite is not None and imgWhite.size > 0:
            print(f"Saving image to: {folder}/Image_{time.time()}.jpg")
            cv2.imwrite(f"{folder}/Image_{time.time()}.jpg", imgWhite)
            counter += 1
            print(f"Image saved: {counter}")
        else:
            print("Error: imgWhite is invalid.")
